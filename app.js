...
// 통합 saveState: 로컬만 즉시 저장 (구글 시트는 버튼으로만)
function saveState() {
  saveLocal();
...
// 현재 로컬 데이터를 구글 시트로 강제 이전
async function migrateToSheet() {
  if (!confirm('현재 브라우저에 저장된 데이터를 구글 시트로 업로드합니다.\n구글 시트의 기존 데이터는 덮어씌워집니다. 계속하시겠습니까?')) return;
  ...
  setTimeout(() => {
    btn.textContent = '☁️ 현재 데이터 → 구글 시트로 이전';
    btn.disabled = false;
  }, 3000);
  showToast('구글 시트로 이전 완료!', 'success');
}
...
async function forceSyncFromSheet() {
  ...
  if (ok) {
    ...
    loadDailyData();
    renderStats();
    showToast('구글 시트에서 불러오기 완료', 'success');
  } else {
    showToast('구글 시트 연결 실패. 로컬 데이터를 사용합니다.', 'error');
  }
}
...
const DEFAULT_EMPLOYEES = [
  {id:1, name:'권양구', div:'관리', home:''},
  {id:2, name:'김광수', div:'공사', home:'정원화학'},
  {id:3, name:'권오근', div:'공사', home:'정원화학'},
  {id:4, name:'민영길', div:'공사', home:'리뉴시스템'},
  {id:5, name:'강민재', div:'가공', home:'DFC'},
  {id:6, name:'강용구', div:'제관', home:''},
  {id:7, name:'송기남', div:'관리', home:''},
  {id:8, name:'신동범', div:'보조', home:'ASK'},
  {id:9, name:'최병유', div:'용접', home:'리뉴시스템'},
  {id:10,name:'서석현', div:'공사', home:'인도네시아'},
  {id:11,name:'김두용', div:'가공', home:'DFC'},
  {id:12,name:'석병찬', div:'보조', home:'정원화학'},
  {id:13,name:'신동진', div:'가공', home:'DFC'},
  {id:14,name:'차재영', div:'가공', home:'DFC'},
  {id:15,name:'최가람', div:'관리', home:''},
  {id:16,name:'강희석', div:'보조', home:'ASK'},
  {id:17,name:'구은서', div:'공사', home:'ASK'},
  {id:18,name:'김경호', div:'제관', home:'정원화학'},
  {id:19,name:'김기동', div:'용접', home:'리뉴시스템'},
  {id:20,name:'이상근', div:'용접', home:'리뉴시스템'},
  {id:21,name:'홍성준', div:'용접', home:'정원화학'},
  {id:22,name:'김성기', div:'제관', home:''},
  {id:23,name:'민영도', div:'보조', home:'리뉴시스템'},
  {id:24,name:'김익탁', div:'공사', home:'인도네시아'},
  {id:25,name:'하동윤', div:'제관', home:'정원화학'},
  {id:26,name:'임혁종', div:'가공', home:'DFC'}
];
  ...
  state.employees.forEach(emp => {
    ...
    card.innerHTML =
      ...
      '<div class="emp-division">' +
        ...
        '</select>' +
      '</div>' +
      '<input type="text" value="' + (emp.home||'') + '" placeholder="출장지" style="width:90px;font-size:11px;" onchange="updateEmpHome(' + emp.id + ', this.value)">' +
...
function addEmployee() {
  ...
  state.employees.push({ id: maxId+1, name, div, home });
  saveState();
  renderEmployees();
...
function addProject() {
  ...
  // [I4] 구매관리 통합용 필드 포함하여 생성
  state.projects.push({ id: maxId+1, code, client, title, site, manager, phone:'', claimPrefix });
  saveState();
  renderProjects();
...
function loadDailyData() {
  ...
  const dow = DAYS_KO[new Date(date+'T00:00:00').getDay()];
  document.getElementById('day-of-week').textContent = `(${dow}요일)`;
...
function renderEmpInputGrid(empData) {
  ...
  sorted.forEach(emp => {
    ...
    const d = DIVISIONS[emp.div] || { label:emp.div, cls:'' };
    const isAbsent = ed.status === '휴가' || ed.status === '연차';
    ...
    const statusBtns = ['출근','반차','연차','휴가'].map(s =>
      '<button class="status-btn ' + (ed.status===s?'active-'+s:'') + '" onclick="setEmpStatus(' + emp.id + ',\'' + s + '\',this)">' + (s==='휴가'?'休':s) + '</button>'
    ).join('');

    // 투입 프로젝트 옵션
    const projOpts = state.projects.map(p =>
      '<option value="' + p.id + '" ' + (String(ed.projId)===String(p.id)?'selected':'') + '>' +
    ...
    row.innerHTML =
      ...
      '<div class="emp-row-controls">' +
        ...
        '<div class="emp-work-input">' +
          '<input type="text" value="' + (ed.work||'').replace(/"/g,'&quot;') + '" placeholder="작업 내용 / 휴가 사유 (공상, 산재, 경조사, 기타...)" onchange="setEmpWork(' + emp.id + ',this.value)">' +
        '</div>' +
      '</div>';
...
function setEmpStatus(id, status, btn) {
  ...
  row.querySelectorAll('.status-btn').forEach(b => {
    ...
    const s = b.textContent === '休' ? '휴가' : b.textContent;
    if (s === status) b.classList.add('active-'+status);
  });
  row.classList.toggle('is-absent', status === '휴가' || status === '연차');
...
function rerenderProjTotals(projId) {
  ...
  state.employees.forEach(emp => {
    const ed = empData[emp.id] || { status:'출근', projId:'' };
    if (ed.status === '휴가' || ed.status === '연차') return;
    if (String(ed.projId) !== String(projId)) return;
    const mpCat = DIV_TO_MP[emp.div];
    if (mpCat) auto[mpCat]++;
  });
...
function buildReportHTML(date) {
  ...
  state.employees.forEach(emp => {
    ...
    const otH = ed.overtimeHours || (ed.overtime ? 2.5 : 0);
    if (otH > 0 && ed.status !== '휴가') overtimeEmps.push({ name: emp.name, hours: otH });
    const isOnTrip = ed.onTrip || ed.trip;
    if (isOnTrip && ed.status !== '휴가') {
...
function renderStats() {
  ...
  if (summaryEl) {
    summaryEl.innerHTML =
      ...
      '<div class="stats-card"><div class="s-val" style="color:var(--red)">' + totalAbsent + '</div><div class="s-lbl">결근·연차 (인·일)</div></div>' +
      (totalHalfDay > 0
...
function printMonthlyStats() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    state.employees.forEach(emp => {
      ...
      if (ed.status === '휴가' || ed.status === '연차') { totalAbsent++; return; }
      dayPresent++;
  ...
  for (let d=1; d<=daysInMonth; d++) {
    ...
    if (data) {
      ...
      state.employees.forEach(emp => {
        ...
        if (ed.status === '휴가' || ed.status === '연차') absent++;
        else present++;
      });
    }
...
function renderWoProjBlock(container, proj, emps, woData, date) {
  ...
  if (emps.length === 0) {
    ...
    emps.forEach(emp => {
      ...
      const projOpts = state.projects.map(p =>
        '<option value="' + p.id + '"' + (String(p.id)===String(proj.id)?' selected':'') + '>' + p.client + (p.code?' ('+p.code+')':'') + '</option>'
      ).join('');
...
function renderWoUnassignedBlock(container, unassignedList, woData, date) {
  ...
  const projOpts = state.projects.map(p =>
    '<option value="' + p.id + '">' + p.client + (p.code?' ('+p.code+')':'') + '</option>'
  ).join('');
...
function renderDashboard() {
  ...
  const kpiHTML =
    ...
    '<div class="dashboard-kpi-card" onclick="switchToTab(\'daily\')" title="④ 일일 입력으로 이동">' +
      ...
      '<div class="dashboard-kpi-lbl">결근·휴가</div>' +
    '</div>' +
    '<div class="dashboard-kpi-card" onclick="switchToTab(\'daily\')" title="④ 일일 입력으로 이동">' +
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (hasData) {
      ...
      state.employees.forEach(emp => {
        ...
        if (ed.status !== '휴가' && ed.status !== '연차') miniPresent++;
      });
    }
...
function showEmpDetail(empId) {
  ...
  for (let d = daysInMonth; d >= 1; d--) {
    ...
    if (ed && ed.status !== '휴가' && ed.status !== '연차') {
      const otH = ed.overtimeHours || 0;
      ...
      } else if (ed.status !== '휴가' && ed.status !== '연차') {
        unassignedDays++;
      }
    }
...
/**
 * 프로젝트 드롭다운 갱신 (state.projects 전체 표시)
 */
function pr_populateProjectSelect() {
  ...
  sel.innerHTML = '<option value="">-- 프로젝트 선택 --</option>' +
    state.projects.map(p =>
      '<option value="' + p.id + '"' + (String(p.id) === currentVal ? ' selected' : '') + '>' +
      p.client + (p.code ? ' (' + p.code + ')' : '') + '</option>'
    ).join('');
}

/**
 * 프로젝트 선택 시 자동 입력 (담당자, 현장, 전화번호, 청구번호)
 */
function pr_onProjectChange() {
  ...
  setVal('pr-phone',    proj.phone);

  // 청구번호 자동 생성: prefix + 시퀀스
  if (proj.claimPrefix) {
    const seq = String(state.purchaseDB.length + 1).padStart(2, '0');
    setVal('pr-claim-no', proj.claimPrefix + '-' + seq);
  }
}
...
function pr_renderDB() {
  ...
  tbody.innerHTML = db.map((row, i) => {
    ...
    return '<tr>' +
      ...
      '<td><button class="btn btn-ghost btn-sm" style="color:var(--red);padding:2px 6px;font-size:10px;" onclick="pr_deleteRow(' + actualIdx + ')">삭제</button></td>' +
      '</tr>';
  }).join('');
}
